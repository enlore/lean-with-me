...
class LeanKitBoard {
        ...
        function onResp (resp) {
            ...
            if (resp.statusCode >= 400) {
                ...
                if (opts.streamed) return cb(null, resp);
                    ...
                    resp.on("data", chunk => this._doChunk(uri, chunk));
                    resp.on("end", chunk => { this._doEnd(uri, resp, cb); });
    ...
    _doEnd (uri, resp, cb) {
        ...
        if (ct && /json/.test(ct)) {
            try {
                let data = JSON.parse(this._resp[uri]);

                this._sortLeanKitResponse(uri, resp, data, cb);
                ...
                if (this.debug) {
                    ...
                    process.stderr.write(this._resp[uri]);
                    process.stderr.write("\n\n\n\n");
                }
        ...
        // Every other kind of body
            ...
            cb(null, this._resp[uri]);
            this._resp[uri] = "";
        }
    }

    _sortLeanKitResponse (uri, resp, lkResp, cb) {
        ...
        if (code >= 500) {
            ...
            err._lkRespBody = this._resp[uri];
